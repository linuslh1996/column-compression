...
run_benchmark() {
    ...
    if cmake .. -DCMAKE_C_COMPILER=clang-$clang_version -DCMAKE_CXX_COMPILER=clang++-$clang_version -DCMAKE_BUILD_TYPE=Release -DHYRISE_RELAXED_BUILD=On -GNinja && ninja "$benchmark_name" ; then
        perf stat -e "$to_measure" -o ../cache_results/"$2"_baseline_sf"$scale_factor"_single.txt ./"$benchmark_name" -e ../encoding_$2.json --dont_cache_binary_tables -r 1 -s $scale_factor --scheduler --clients $max_clients --mode=Shuffled
        perf stat -e "$to_measure" -o ../cache_results/"$2"_baseline_sf"$scale_factor"_multi.txt ./"$benchmark_name" -e ../encoding_$2.json --dont_cache_binary_tables -r 1 -s $scale_factor >> ./cache_results/sizes_$2.txt

        perf stat -e "$to_measure" -o ../cache_results/"$2"_cache_sf"$scale_factor"_single.txt ./"$benchmark_name" -e ../encoding_$2.json --dont_cache_binary_tables -o ../cache_results/tpch_$2_28_shuffled.json -t $max_time -s $scale_factor --scheduler --clients $max_clients --mode=Shuffled
        perf stat -e "$to_measure" -o ../cache_results/"$2"_cache_sf"$scale_factor"_multi.txt ./"$benchmark_name" -e ../encoding_$2.json --dont_cache_binary_tables -o ../cache_results/tpch_$2_singlethreaded.json -r 300
    fi
    cd ..
}
